...
class StateMachine(LeafSystem):
    ...
    def __init__(
        ...
        self.X1_preskewer = RigidTransform(
            RotationMatrix(RollPitchYaw(np.deg2rad([180, 90, 0]))), [0.4, 0.4, 0.4]
        )
        self.X2_preskewer = RigidTransform(
            RotationMatrix(RollPitchYaw(0, 0, 0)), [0.4, -0.1, 0.4]
        )
        self.X1_skewered = RigidTransform(
            RotationMatrix(RollPitchYaw(np.deg2rad([180, 90, 0]))), [0.4, 0.19, 0.4]
        )
        self.X2_released = RigidTransform(
            RotationMatrix(RollPitchYaw(0, 0, 0)), [0.4, -0.5, 0.4]
        )

        # State machine setup
        self._state1_index = self.DeclareAbstractState(AbstractValue.Make(State.START))
        self._state2_index = self.DeclareAbstractState(AbstractValue.Make(State.START))
        ...
        for i in range(num_cameras):
            self.DeclareAbstractInputPort(
                f"camera{i}_point_cloud", AbstractValue.Make(PointCloud())
            )
        ...
        self.iiwa1_time = self._plant_context1.get_time()
        self.iiwa2_time = self._plant_context2.get_time()
        self.skewer_done = False

        self._drawGrasp(self.X1_preskewer, "target pre-skewer state")
        self._drawGrasp(self.X1_skewered, "target skewer state")

    def CalcDesiredPose1(self, context, output):
        state = self._GetState(iiwa_num=1, context=context)
        time = context.get_time()
        if time - self.iiwa1_time >= 0.01:
            self.iiwa1_time = time
            match state:
                case State.START:
                ...
                case State.GET_TO_SKEWER_POSITION:
                    ...
                    if (
                        self._AtPose(context, iiwa_num=1, X=self.X1_preskewer)
                        and self._AtPose(context, iiwa_num=2, X=self.X2_preskewer)
                        and self._GetState(iiwa_num=2, context=context)
                        == State.GET_TO_SKEWER_POSITION
                    ):
    ...
    def CalcDesiredPose2(self, context, output):
        ...
        gripper_state = context.get_abstract_state(
            int(self._gripper_state_index)
        ).get_value()
        time = context.get_time()
        if time - self.iiwa2_time >= 0.01:

            self.iiwa2_time = time
            match state:
                case State.START:
                    ...
                    if self._AtPose(context, iiwa_num=2, X=self.X2_preskewer):
                        ...
                        for i in range(self.num_cameras):
                            pcds.append(
                                self.GetInputPort(f"camera{i}_point_cloud").Eval(
                                    context
                                )
                            )
                        ...
                        if self.meshcat is not None:
                            self._drawGrasp(
                                self.X_PregraspGrasp @ X_desired,
                                f"grasp{self.grasp_count}",
                            )
                            self.grasp_count += 1
                    ...
                    if self._AtPose(context, iiwa_num=2, X=self.X2_desired):
                        ...
                        if self._Timeout(1, iiwa_num=2, context=context):
                            ...
                            if gripper_state == self.gripper_open:
                                ...
                                self.X2_desired = self.X2_preskewer
                                self._ChangeState(
                                    2, State.GET_TO_SKEWER_POSITION, context
                                )
                ...
                case State.GET_TO_SKEWER_POSITION:
                    ...
                    if self._AtPose(
                        context, iiwa_num=2, X=self.X2_preskewer
                    ) and self._AtPose(context, iiwa_num=1, X=self.X1_preskewer):
                        # Move to next state
    ...
    def _AtPose(
        self, context: Context, iiwa_num: int, X: RigidTransform, tol: float = 0.1
    ) -> bool:
        """
        ...
        if iiwa_num == 1:
            # Update plant
            self._plant1.SetPositions(
                self._plant_context1, self._iiwa1, self._q1_in.Eval(context)
            )

            # Current pose of the end effector
            X_now = self._plant1.CalcRelativeTransform(
                self._plant_context1, self._plant1.world_frame(), self._G1
            )
            ...
            # Update plant
            self._plant2.SetPositions(
                self._plant_context2, self._iiwa2, self._q2_in.Eval(context)
            )

            # Current pose of the end effector
            X_now = self._plant2.CalcRelativeTransform(
                self._plant_context2, self._plant2.world_frame(), self._G2
            )
    ...
    def _Timeout(self, duration: float, iiwa_num: int, context: Context) -> bool:
        """
    ...
    def _GetState(self, iiwa_num: int, context: Context) -> State:
        """
        Gets the state of the desired iiwa
        """
        if iiwa_num == 1:
            return context.get_abstract_state(int(self._state1_index)).get_value()
        elif iiwa_num == 2:
            return context.get_abstract_state(int(self._state2_index)).get_value()
        else:
            raise RuntimeError("iiwa_num was invalid")

    def _ChangeState(self, iiwa_num: int, state: State, context: Context) -> None:
        """
    ...
    def _ChangeGripper(self, context, value):
        context.get_mutable_abstract_state(int(self._gripper_state_index)).set_value(
            value
        )
